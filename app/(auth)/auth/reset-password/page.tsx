...
// So create a wrapper component for the main logic
function ResetPasswordForm() {
  ...
  useEffect(() => {
    if (!token) {
      ...
      // Optionally redirect or show a link to forgot-password page
    }
  }, [token]);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        // For now, just show success message and let user click link
      } else {
  ...
  if (!token && !error) { // If token is null from URL initially, useEffect will set error.
      return (
  ...
  if (error && !message) { // Show critical errors prominently if token was bad from start
      return (
        <div className="text-center">
